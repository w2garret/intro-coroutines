...
val testRepos =
    listOf(
        TestRepo("repo-1", 1000, listOf(User("user-1", 10), User("user-2", 20))),
        TestRepo("repo-2", 1200, listOf(User("user-2", 30), User("user-1", 40))),
        TestRepo("repo-3", 800, listOf(User("user-2", 50), User("user-3", 60))))
...
val expectedResults =
    TestResults(
        ...
        listOf(User("user-2", 100), User("user-3", 60), User("user-1", 50)))

val expectedConcurrentResults =
    TestResults(
...
val progressResults =
    listOf(
        TestResults(
            2000, // 1000 + 1000
            listOf(
                User(login = "user-2", contributions = 20),
                User(login = "user-1", contributions = 10))),
        TestResults(
            3200, // 2000 + 1200
            listOf(
                User(login = "user-2", contributions = 50),
                User(login = "user-1", contributions = 50))),
        expectedResults)

val concurrentProgressResults =
    listOf(
        TestResults(
            1800, // 1000 + 800
            listOf(
