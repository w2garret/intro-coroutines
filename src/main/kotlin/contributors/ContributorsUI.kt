...
class ContributorsUI : JFrame("GitHub Contributors"), Contributors {
  ...
  private val resultsScroll = JScrollPane(results).apply { preferredSize = Dimension(200, 200) }
  ...
  override fun updateContributors(users: List<User>) {
    ...
    resultsModel.setDataVector(
        users.map { arrayOf(it.login, it.contributions) }.toTypedArray(), COLUMNS)
  }
...
fun JPanel.addWideSeparator() {
  addWide(JSeparator()) { fill = GridBagConstraints.HORIZONTAL }
}
